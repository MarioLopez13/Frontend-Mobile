...
class NfcPaymentConfirmPage extends ConsumerStatefulWidget {
  final NfcPaymentPayload payload;

  const NfcPaymentConfirmPage({
    super.key,
    required this.payload,
  });

  @override
  ConsumerState<NfcPaymentConfirmPage> createState() =>
      _NfcPaymentConfirmPageState();
}

class _NfcPaymentConfirmPageState
    extends ConsumerState<NfcPaymentConfirmPage> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _handleConfirmPayment() async {
    final result = await ref
        ...
        .payWithNfc(payload: widget.payload);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Confirmar pago NFC'),
      ),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          const Text(
            'Revisa la información antes de confirmar la transacción NFC.',
            style: TextStyle(
              fontSize: 16,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _SummaryRow(
                    label: 'Referencia',
                    value: widget.payload.tagReference,
                  ),
                  const SizedBox(height: 12),
                  _SummaryRow(
                    label: 'Monto',
